...
public class Limit extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		task = new phpDown();
		task.execute("http://pty1026.cafe24.com/ordUnitList/selectMixLimit.php","00");
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				builder.setTitle(unit_obj[position].getMain_unit())
						.setMessage(unit_obj[position].getSub_unit1()+"\n"
								...
								+unit_obj[position].getSub_unit6()+"\n"
								+unit_obj[position].getSub_unit7()+"\n")
		...
		protected void onPostExecute(String str){
			...
			try{
				...
				for(int i=0;i<ja.length();i++){
					...
					//Log.d("phpDown","onPostExecute TRY //"+ unit_obj[i].getFrequently());
